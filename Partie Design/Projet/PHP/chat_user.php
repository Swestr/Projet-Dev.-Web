...
for($chat=0;$chat<$mes_chats->size();$chat++) {
		...
		echo '<div id="afficherMsg_box" style="right:'.$right.'px">
			...
			for ($msg=0; $msg <$mes_chats->getChat($chat)->size() ; $msg++) {
							echo '<p>'.$mes_chats->getChat($chat)->getMessage($msg)->getJoueur()->getPseudo().' : '.$mes_chats->getChat($chat)->getMessage($msg)->getMessage().' ('.$mes_chats->getChat($chat)->getMessage($msg)->getDate().')</p>';

			}
		echo '</div><div class="msg_footer"><textarea class="msg_input" rows="4"></textarea></div></div>
			</div>
			</div>';
}
?>
